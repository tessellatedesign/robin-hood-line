...
class FrmProApplicationRelationHelper {
	...
	private function get_parent_of_data_for_view( $view ) {
		...
		if ( ! empty( $options['before_content'] ) ) {
			$html .= $options['before_content'];
		}
		$html .= $view->post_content;
		if ( ! empty( $options['after_content'] ) ) {
			$html .= $options['after_content'];
		}
	...
	public function handle_shortcode( $shortcode_found, $id_found ) {
		switch ( $shortcode_found ) {
			case 'formidable':
				if ( array_key_exists( $id_found, $this->form_names_by_id ) ) {
					return $this->map_form_id_to_info_array( $id_found );
				}
				break;
			case 'display-frm-data':
				if ( array_key_exists( $id_found, $this->post_titles_by_id ) ) {
					return $this->map_view_id_to_info_array( $id_found );
				}
				break;
		}
		return false;
